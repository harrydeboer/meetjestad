...
start_date = datetime.datetime.strptime((os.getenv('START_DATE')),"%Y-%m-%d,%H:%M:%S").year
end_year = datetime.datetime.strptime((os.getenv('END_DATE')),"%Y-%m-%d,%H:%M:%S").year
        ...
        if year == end_year:
            if quarter == '1':
                if year % 4 == 0 and (year % 100 != 0 or year % 400 == 0):
                    end_date = str(year) + '-02-29,22:59:59'
                else:
                    end_date = str(year) + '-02-28,22:59:59'
            elif quarter == '2':
                end_date = str(year) + '-05-31,21:59:59'
            elif quarter == '3':
                end_date = str(year) + '-08-31,21:59:59'
            elif quarter == '4':
                end_date = str(year) + '-11-30,22:59:59'
            else:
